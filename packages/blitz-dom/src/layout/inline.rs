            ...
            while let Some(yield_data) = breaker.break_next() {
                match yield_data {
                    ...
                    YieldData::InlineBoxBreak(box_break_data) => {
                        ...
                        // `pos` is content-box-relative: the sub-context created with
                        // `container_pb` offsets strip the container's padding/border
                        // from the returned position. It is converted to a border-box
                        // location below (with the margin), like the non-floated inline
                        // box branch — adding `container_pb` here as well double-counted
                        // the container's padding, drawing the float too low/inset.
